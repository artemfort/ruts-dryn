    ...
    if (typeof io !== 'undefined') {
       ...
       socket.on('connect_error', (error) => {
        ...
        socket.on('new-message', (data) => {
            ...
            if (currentChannel && currentChannel.id === channelId && currentView === 'server') {
            ...
            if (document.hidden) {
                ...
                const title = channel ? `#${channel.name}` : 'New Message';
                showNotification(title, `${message.author}: ${message.text}`);
            }
        });
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (Array.isArray(server.channels)) {
                channelsByServer.set(server.id, server.channels);
                server.channels.forEach(channel => channelStore.set(channel.id, channel));
            }
...
function initializeChannelControls() {
    ...
    if (addTextChannelBtn) {
        addTextChannelBtn.addEventListener('click', () => promptChannelCreation('text'));
    }

    if (addVoiceChannelBtn) {
        addVoiceChannelBtn.addEventListener('click', () => promptChannelCreation('voice'));
    }
}

async function promptChannelCreation(type) {
    if (!currentServerId) {
        alert('Select a server first');
        return;
    }

    const channelName = prompt(`Enter ${type} channel name:`)?.trim();
    ...
    try {
        ...
        const list = channelsByServer.get(currentServerId) || [];
        list.push(channel);
        channelsByServer.set(currentServerId, list);
        renderChannelList(currentServerId);
    } catch (error) {
...
async function loadServerChannels(serverId) {
    ...
    try {
        ...
        const normalizedChannels = fetchedChannels.map(channel => ({
            ...channel,
            serverId: channel.serverId || serverId
        }));

        channelsByServer.set(serverId, normalizedChannels);
        normalizedChannels.forEach(channel => channelStore.set(channel.id, channel));
        renderChannelList(serverId);

        if (normalizedChannels.length === 0) {
            currentChannel = null;
            document.getElementById('currentChannelName').textContent = 'No channel';
            const chatHeaderInfo = document.getElementById('chatHeaderInfo');
            if (chatHeaderInfo) {
                chatHeaderInfo.innerHTML = `
                    <span class="channel-name">No channels</span>
                    <span class="channel-topic">Create one to start chatting</span>
                `;
            }
            const messagesContainer = document.getElementById('messagesContainer');
            if (messagesContainer) {
                messagesContainer.innerHTML = '<div class="messages-empty">No channels available.</div>';
            }
            return;
        }

        const hasCurrentChannel = currentChannel && normalizedChannels.find(c => c.id === currentChannel.id);
        if (!hasCurrentChannel) {
            const defaultChannel = normalizedChannels.find(c => c.type === 'text') || normalizedChannels[0];
            if (defaultChannel) {
...
function renderChannelList(serverId) {
    ...
    const channelList = channelsByServer.get(serverId) || [];
    const textChannels = channelList.filter(channel => channel.type === 'text');
    const voiceChannels = channelList.filter(channel => channel.type === 'voice');

    if (textChannels.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'channel-empty';
        empty.textContent = 'No text channels yet';
        textList.appendChild(empty);
    } else {
        textChannels.forEach(channel => textList.appendChild(createChannelElement(channel)));
    }

    if (voiceChannels.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'channel-empty';
        empty.textContent = 'No voice channels yet';
        voiceList.appendChild(empty);
    } else {
        voiceChannels.forEach(channel => voiceList.appendChild(createChannelElement(channel)));
    }
}
...
function createChannelElement(channel) {
    ...
    channelEl.className = `channel ${isVoice ? 'voice-channel' : 'text-channel'}`;
    channelEl.dataset.channelId = channel.id;
    if (currentChannel && currentChannel.id === channel.id) {
        channelEl.classList.add('active');
    }
    if (activeVoiceChannel && activeVoiceChannel.id === channel.id) {
        channelEl.classList.add('in-call');
    }

    channelEl.innerHTML = `
        ${isVoice ? VOICE_CHANNEL_ICON : TEXT_CHANNEL_ICON}
...
function switchChannel(channel) {
    ...
    const currentEl = document.querySelector(`.text-channel[data-channel-id="${channel.id}"]`);
    ...
    if (chatHeaderInfo) {
        chatHeaderInfo.innerHTML = `
            ...
            <span class="channel-topic">Text Channel</span>
        `;
    }
...
async function loadChannelMessages(channelId) {
    ...
    try {
        ...
        channelMessages.set(channelId, normalizedMessages);
        normalizedMessages.forEach(addMessageToUI);
        scrollToBottom();
    } catch (error) {
...
function sendMessage() {
    ...
    if (socket && socket.connected) {
        ...
        } else if (currentView === 'server' && currentChannel) {
...
function addMessageToUI(message) {
    ...
    if (message.file) {
        const attachment = renderFileAttachment(message.file);
        if (attachment) {
...
function renderFileAttachment(file) {
    ...
    attachment.className = 'message-file';
    ...
    fileName.textContent = file.filename || 'Attachment';
    ...
    if (isImageFile(file.type)) {
        ...
        preview.alt = file.filename || 'Attachment preview';
...
async function uploadFile(file) {
    try {
        if (!currentChannel) {
            alert('Select a text channel first.');
            return;
        }
...
async function joinVoiceChannel(channel) {
    ...
    const channelEl = document.querySelector(`.voice-channel[data-channel-id="${channel.id}"]`);
    if (channelEl) channelEl.classList.add('in-call');
    
    const callInterface = document.getElementById('callInterface');
    callInterface.classList.remove('hidden');
    
    document.querySelector('.call-channel-name').textContent = channel.name;
...
function leaveVoiceChannel(force = false) {
    ...
    if (force) {
        inCall = false;
        const channelId = activeVoiceChannel ? activeVoiceChannel.id : null;
        const serverId = activeVoiceChannel ? activeVoiceChannel.serverId : null;
