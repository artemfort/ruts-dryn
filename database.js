...
function ensureFileUploadMessageColumn() {
    db.all('PRAGMA table_info(file_uploads)', (err, columns) => {
        if (err) {
            console.error('Failed to inspect file_uploads schema:', err);
            return;
        }

        const hasMessageId = columns.some(column => column.name === 'message_id');
        if (!hasMessageId) {
            db.run('ALTER TABLE file_uploads ADD COLUMN message_id INTEGER', (alterErr) => {
                if (alterErr) {
                    console.error('Failed to add message_id column:', alterErr);
                } else {
                    console.log('Added message_id column to file_uploads table');
                }
            });
        }
    });
...
// File operations
const fileDB = {
    create: (filename, filepath, filetype, filesize, userId, channelId, messageId = null) => {
        return new Promise((resolve, reject) => {
            const sql = 'INSERT INTO file_uploads (filename, filepath, filetype, filesize, user_id, channel_id, message_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
            db.run(sql, [filename, filepath, filetype, filesize, userId, channelId, messageId], function(err) {
                if (err) reject(err);
                else resolve({ id: this.lastID, filename, filepath, filetype, filesize, channelId, messageId });
            });
        });
    },

    attachToMessage: (fileId, messageId) => {
        return new Promise((resolve, reject) => {
            const sql = 'UPDATE file_uploads SET message_id = ? WHERE id = ?';
...
const channelDB = {
    ...
    getByServer: (serverId) => {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT id, name, type, server_id as serverId
                FROM channels
                WHERE server_id = ?
