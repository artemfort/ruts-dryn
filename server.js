...
function buildVoiceRoomKey(serverId, channelId) {
...
app.post('/api/upload', authenticateToken, upload.single('file'), async (req, res) => {
    try {
        ...
        await fileDB.attachToMessage(fileRecord.id, savedMessage.id);

        const user = await userDB.findById(req.user.id);
        const filePayload = {
            id: fileRecord.id,
            filename: req.file.originalname,
            url: `/uploads/${path.basename(fileRecord.filepath)}`,
        ...
        const broadcastMessage = {
            ...
            avatar: user.avatar || user.username.charAt(0).toUpperCase(),
            text: `Uploaded ${req.file.originalname}`,
            file: filePayload,
            timestamp: new Date()
        };
...
app.get('/api/messages/:channelId', authenticateToken, async (req, res) => {
    try {
        ...
        const messages = await messageDB.getByChannel(channelId);
        const formattedMessages = messages.map((message) => {
            ...
            if (message.file_id) {
                payload.file = {
                    ...
                    url: message.file_path ? `/uploads/${path.basename(message.file_path)}` : null,
        ...
        try {
            channels = await Promise.all(
                DEFAULT_CHANNELS.map(channel =>
                    channelDB.create(channel.name, channel.type, server.id)
                )
            );
        } catch (channelError) {
            console.error('Default channel creation failed:', channelError);
        }
...
app.get('/api/servers/:serverId/channels', authenticateToken, async (req, res) => {
    try {
        ...
        const isMember = await serverDB.isMember(serverId, req.user.id);
        if (!isMember) {
            return res.status(403).json({ error: 'Not a member of this server' });
        }

        let channels = await channelDB.getByServer(serverId);

        if (channels.length === 0) {
            try {
                await Promise.all(
                    DEFAULT_CHANNELS.map(channel =>
                        channelDB.create(channel.name, channel.type, serverId)
                    )
                );
                channels = await channelDB.getByServer(serverId);
            } catch (seedError) {
                console.error('Failed to seed default channels:', seedError);
            }
        }
...
app.post('/api/servers/:serverId/channels', authenticateToken, async (req, res) => {
    try {
        ...
        const channel = await channelDB.create(name.trim(), normalizedType, serverId);
...
io.on('connection', async (socket) => {
    ...
    socket.on('send-message', async (messageData) => {
        try {
            ...
            const channel = await ensureChannelAccess(numericChannelId, socket.userId);
    ...
    socket.on('join-voice-channel', async (channelData) => {
        try {
            const { channelId, serverId, userId } = channelData;
            const roomKey = buildVoiceRoomKey(serverId, channelId);
            if (!roomKey) return;

            const hasAccess = await serverDB.isMember(serverId, socket.userId);
            if (!hasAccess) {
                return;
            }
            ...
            socket.to(roomKey).emit('user-joined-voice', {
                userId,
                socketId: socket.id
            });
    ...
    socket.on('leave-voice-channel', (channelData) => {
        const { channelId, serverId } = channelData || {};
        const roomKey = buildVoiceRoomKey(serverId, channelId);
        if (!roomKey) return;
        ...
        if (rooms.has(roomKey)) {
            rooms.get(roomKey).delete(socket.id);
    ...
    socket.on('disconnect', async () => {
        ...
        if (user) {
            ...
            rooms.forEach((members, roomKey) => {
                if (members.has(socket.id)) {
                    members.delete(socket.id);
                    io.to(roomKey).emit('user-left-voice', socket.id);
                    if (members.size === 0) {
                        rooms.delete(roomKey);
                    }
                }
